...
//인자로 주어진 position의 부모 노드를 찾아 주소를 리턴하는 함수
BinTreeNode *find_parent(BinTree *tree,char *position)
{
    BinTreeNode *node = NULL;//부모 노드의 주소 넣을 포인터
    int str_len = 0;
    int i = 0;
...
//특정 위치의 노드 주소를 반환하는 함수.
BinTreeNode *getNode(BinTree *tree,char *position)
{
    BinTreeNode *node = NULL;//부모 노드의 주소를 저장할 포인터
    BinTreeNode *preturn = NULL;//찾는 노드의 주소를 반환할 포인터
...
//인자로 받은 트리의 특정 위치에 value값을 가지는 이진 노드를 하나 삽입하는 함수.
int     putNode(BinTree *tree,char *position, char value)
{
    ...
    BinTreeNode ele;//value값을 넣어 만들 이진 노드
